...
## 验证约定 · 浏览器取证（2026-09-02 追加）

- 凡「前端显示 / 配置不生效 / 页面看不到某内容」类问题，**必须主动拉起浏览器实测取证并截图**，禁止只停留在"读代码 + curl + 翻数据库"的纸面层判断。
- 静态代码审查与运行时真实渲染之间隔着登录态、Service Worker 缓存、前端构建版本等屏障，只有端到端浏览器实测才能暴露。曾因此屡漏检 minimax provider 显示问题。
- 遇到此类问题**不预先汇报、不询问**，直接自拉浏览器：真实账号登录（admin@yexingchen.cn，密码在项目根 `.secrets/local.env`）→ 打开目标页 → 截图 → 用事实结论回复。
- 判断"是否真显示/真生效"以浏览器实际截图为准，不以代码该不该显示或 SQL 里有没有记录为准。
- 线上主库 `/var/www/yexingchen/backend/yexingchen.db`（sqlite）；SSH `root@203.195.208.25`，密码在 `.secrets/local.env`。

## 文档沉淀约定（防断档）

- 产品资料与设计文档统一放在 `D:\software\codex\codex_work\docs\ai`，后续智能体先读该目录再接续工作。
- **任何设计/界面相关的审视、诊断、方案决策、验收，都必须先产出一份带日期的 `docs/ai/*.md` 文档，再动手改代码**，保证后续智能体从文档接续、不依赖本会话上下文。
- 修改界面/配色/字体/动效前，先读 `docs/DESIGN_INKWASH.md`（唯一有效设计规范）与最近的设计审视/方案文档。
- 现有设计审视基线见 `docs/ai/design-review-2026-09-02-workbench-upgrade.md`。
