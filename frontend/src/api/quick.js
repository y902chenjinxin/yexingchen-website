/**
 * 闪念速记 API
 * 与 finance.js 同构：独立 axios 实例，不自动解包 data，调用方读 res.data.*。
 */
...
export const quickApi = {
  create: (content) => api.post('/quick', { content }),
  list: (params = {}) => api.get('/quick', { params }),
  remove: (id) => api.delete(`/quick/${id}`),
}
