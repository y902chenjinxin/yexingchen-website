...
- **v1.8.0 又犯了同样的错**：跳过了 Step 8 本地预览验证，直接部署到生产

**复发模式**：
- Step 8 自测（本地验证）→ 每次都跳过
- 心里想"代码改了就上线吧，用户等着看效果"
- 结果：未验证的代码直接面向用户

**强制门控（必须执行）**：
1. npm run build 完成后 → 必须 npm run preview 确认
2. 本地预览地址打开确认效果OK → 才能 git commit
3. Git commit + tag → 才能部署到服务器
4. 部署后 curl 验证 HTTP 200 → 才能通知用户验收
