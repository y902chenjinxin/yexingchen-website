...
- **2026-09-19**：**修复：网页端返回按钮「消失 + 失效」双问题（SW `xuanhuang-v117`，User「有些页面的返回按钮失效/消失了（是网页的，手机端我不需要返回按钮）」）**——**踩坑速查新记**：全局 60px 固定顶栏（`GlobalTopBar` z-index:1000）悬浮，凡**非 `IslandInnerBase` 包裹的独立页 / 共享外壳 `.inner-header`**，若顶部 `padding-top` < 60px，页首返回按钮会被顶栏盖住、表现成「返回按钮消失」——**凡「页面顶部返回钮看不到」先查根容器 padding-top 是否 ≥60px（统一 84px），而非只怀疑缓存**；返回逻辑若桌面端写死 `router.push('/workbench')`，从详情进列表再返回会「失效」硬跳工作台——**应统一「有历史 `history.back()`、无历史才回工作台」**。本次：`IslandInnerBase` `.inner-header` 桌面 padding 26→84px（1100px 断点同步 80px）、`goBack()` 简化为回来源页优先（桌面/移动一致）；`ProfileView` 删重复文字版返回、顶部 40→84px。构建核验 `84px 40px 14px` 进 CSS + `history.back` 进 JS + SW v116→v117 →部署 220 文件；**生产桌面浏览器取证 PASS**：/music /tool /finance /stocks /profile /travels /contacts /notes /assistant 九页返回钮均可见（top≥84px 未被顶栏遮挡）、工作台→音乐→返回=工作台 ✓、/stocks→/stocks/600519→返回=**/stocks** ✓（旧逻辑会错跳）、手机端 `.back-btn` 全 `display:none` 符合「手机端不需返回按钮」。仅前端，见 WORK_LOG/CHANGELOG v2.32.0、CURRENT_STATE。
