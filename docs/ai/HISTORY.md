...
- **2026-09-08**：**昼夜自适应主题 + 顶栏/玉简精修 + 全站日间审计（SW `xuanhuang-v65/v66`，见方案 `TIME_THEME_20260908.md`）**——**昼夜主题**：`main.js` 每 60s 按本地时间设 `document.documentElement.dataset.theme`（6–18=`day`，其余 `night`）；`variables.css` 新增 `:root[data-theme="day"]` 覆盖层，背景翻宣纸亮灰、文字深墨阶梯、强调色加深保亮底可读、玻璃家族同步亮化，夜间缺省仍为夜色雨青玻璃。**顶栏/玉简**：顶栏玻璃分层+朱砂 AI 胶囊+搜索+图标上浮；玉简 3s 轮播+篆符对比度+朱砂环+标签托片+激活长胶囊圆点。**第三轮全站审计**：扫出 ~30 处组件**写死深底**（白天透黑块/深底深字）→ 统一日间浅玻璃（笔记选择、灵根测验、K线浮窗、旅行地图卡、岛屿页脚、资源预览、日志/记账/股票/证件照输入框、登录卡+输入框[`html:root` 压过组件内 `!important`]、笔记编辑器取色/代码块/AI 作用域、工具预览框）；功能黑底（视频/照片遮罩）刻意保留。生产 SW v66，169 文件，首页 200；浏览器实测 `data-theme=day` + `.login-card`/输入框 `rgba(255,255,255,.7)`。

- **2026-09-17**：**手机端简化收尾 · P0 骨架 + P1/P2 去古风 + 上线（SW `xuanhuang-v106`，含方案 `MOBILE_SIMPLIFY_20260916.md`）**——新增 `mobile-ab-theme.css`（`#app.is-mobile` 内 A 白天液态玻璃 / B 夜间深空极简双套 token）+ `mobile-deink.css`（P1 隐藏宣纸噪点、`--font-serif` 全局切现代无衬线；P2 账本隐藏 导出/导入`.fin-io`·筛选`.fin-filters`·行内编辑删除`.fin-row-ops`、股票隐藏 `.st-td-edit` 与移动卡删除 `.st-card-h .st-btn.danger`，放大 KPI 大数、KPI 两列），全部门控 `#app.is-mobile`、桌面零回归；重建 `MobileWorkbenchHome`（几何Logo+今日速览+线性图标入口）+ 底部两 Tab 主页/我的。仅前端改动不重部署后端；217 文件 home=200。生产浏览器 DOM 实测去古风 PASS（详见 WORK_LOG 09-17、CHANGELOG v2.25.0）。

- **2026-09-18**：**修复：模块内页白天模式仍深灰（SW `xuanhuang-v116`，User「外面我用白天的样式，进入各个模块里面依然是灰色的，其他模块也一样」→ 全站岛屿内页背景昼夜化）**——**根因**：所有岛屿/工具模块页（音乐/小说/视频/日志/工具/倒计时/音色克隆/水印等）共用 `IslandInnerBase.vue`，其 `.island-inner::before` 背景写死深色渐变 `linear-gradient(168deg,#1b262f 0%,#131b22 55%,#10161c 100%)`，**完全不带 `data-theme`**；而工作台/登录页/岛屿主页(`.island-page`)早有日间覆盖（variables.css 782 行等），唯独这层漏绑 → 外部切白天、进模块仍深灰冷色。**修法**：`variables.css` 新增 `--li-sky/-mid/-deep/--li-title-a` 夜日双套 token（日与 Linear 亮表 `#eef1f3` 同族）；`IslandInnerBase.vue` 背景渐变改 `--li-sky*`、大标题渐变顶改 `--li-title-a`（日深墨、防白字埋亮底），桌面与移动两处标题渐变同步。**验证踩坑**：首个浏览器 agent 报「强制 day 后 `--ls-bg1` 计算仍深色 rgb(11,16,22)」系**测量时机/选错元素**的假警报；改用直接解析 CSS 变量 + 计算 `.island-inner` 与 `::before` 背景后坐实修复生效。构建 `vite build dist2` 核验主 CSS 含 `--li-sky: #1d2830`(夜)/`--li-sky: #eef1f3`(日)、SW v115→v116 → 替换 dist → `deploy_frontend.py` 220 文件 home=200。**生产浏览器取证 PASS**：day 下实测 `--ls-bg1/--li-sky=#eef1f3 /--color-bg=#f6f7f8`，`/tool/countdown` 页面 shell 计算背景 `rgb(244,247,248)`=浅色、截图浅灰白深字；切回 night 仍深色，昼夜切换正常。仅前端，见 CHANGELOG v2.31.0、WORK_LOG 09-18。

- **2026-09-19**：**复验修复三问题（SW `xuanhuang-v121`，v2.33.1）——踩坑速查新记**：① **AI 人像分割输入归一化必须匹配模型训练**：MODNet（及多数语义分割 onnx）训练输入为 **`[-1,1]`**（`/127.5-1`），若前端 WebWorker `mattingWorker.js` 误用 `[0,1]`（`/255`）会把人物判为背景、mask 出力极低、抠不出人。Python 复算：`/255` 人物 mask 占比 7.6%、`/127.5-1` 达 61%——**凡「AI 抠图/分割结果全黑或丢主体」，先核对输入归一化区间**。② **后端部署白名单漏传即「看似修了实为线上跑旧码」**：上轮已改 countdown.py 扩展名校验，但 `deploy_backend.py` `BACKEND_FILES` 未含该文件，线上仍跑旧逻辑 → 改后端后务必确认白名单已含目标文件（本轮补 `routers/countdown.py`+`utils/file_utils.py` 重部署，线上上传 jpg 回 200 PASS）；另前端 `FormData` 上传**勿手写 `Content-Type: multipart/form-data`**（缺 boundary 部分环境解析失败 400），应交浏览器自动生成。③ **SVG 地图城市标记**：原始脚印 glyph(~17×20 地图单位)不缩放会盖住整省，需按地图 viewBox 比例压到城市级（`FOOT_SCALE=0.15`）。本轮：证件照抠图改 `/127.5-1`（人物完整抠出）、倒计时上传白名单+请求头修复（线上实测 PASS，测试图已清）、TravelMap 脚印缩放。见 WORK_LOG/CHANGELOG v2.33.1、CURRENT_STATE。

- **2026-09-19**：**三问题最终闭环 + 证件照后端化收尾（SW `xuanhuang-v126→v127→v128`，v2.34.0→v2.35.1）——踩坑速查新记**：① **onnxruntime-web 的 WASM 后端对动态 INT8 量化算子（ConvInteger 等）输出全零且不报错**——浏览器跑量化分割模型「透明底全 0、人物被抠没」先怀疑量化算子兼容性，最终把推理迁到后端 `onnxruntime` CPU（`routers/idphoto.py`）根治。② **axios 拦截器已 `return response.data` 后，调用处再 `const { data } = await api.post()` 二次解构恒 undefined**——会静默 `return null` 回退旧算法，造成「助手 API 直调通过、用户页面实测不行」的错位；凡拦截器解包的实例，调用处直接用 `body.data`。③ **上传返回 URL 必须与静态挂载前缀对齐**：`save_upload_file` 返 `/countdown/x.jpg` 而静态挂在 `/uploads`（nginx alias），SPA fallback 把它兜底成 HTTP 200 的 index.html「假图」——上传接口返回 URL 时统一补挂载前缀（对齐 travels.py）。④ **`frontend/public/` 下的大文件每次 `vite build` 都会拷进 dist**：模型迁后端后若只删 dist 不删 public 源，下次构建又把 17MB 带回服务器——不用了就删 public 源目录；且部署必需的模型应放 `backend/models/` 并**提交入库**（曾被 gitignore 排除，换机克隆即无法部署）。⑤ **本机 ssh.exe 会无输出挂起**（连上但 banner 后卡死，4 个 job 挂数小时）——远程操作改用 paramiko（同 deploy 脚本，密码在 `.secrets/local.env`），稳定可靠。详见 WORK_LOG/CHANGELOG v2.34.0~v2.35.1、CURRENT_STATE。

- **2026-09-18**：**桌面端「深墨青玉 · Linear 产品风」收敛层（SW `xuanhuang-v115`，User「我不是要你把手机端复制到网页来，我要求的是按照这种风格去设计网页端的 UI，你去找下 github 找类似风格」）**——确立桌面端与手机端是**两种独立设计语言**：手机端为「灵动卡片 + 每模块渐变」的数据工具；桌面端重做为 **Linear 式深色产品风**（参考 Linear/shadcn dark/Supabase dark），实色表面 + 发丝描边层级 + 多层淡阴影造深度，单一青玉 `#4fc6b8` 仅做克制点缀。新增 `desktop-product.css`（471 行，作用域 `#app:not(.is-mobile)`、手机端零回归）：**表面阶梯用边框亮度而非色块**（`--dp-bg #0b0f14`/`--dp-bg2 #0f151b`/`--dp-surface #121820`/`--dp-surface2 #18212b`，发丝线 `--dp-line .10`/`--dp-line-strong .17` 区隔层级）；**Linear 式多层淡阴影** `--dp-shadow=0 1px 2px rgba(0,0,0,.38),0 8px 24px rgba(0,0,0,.22)`（一层紧贴+一层远距柔化、尽量薄），hover 只聚焦描边不加位移/缩放；**青玉克制点缀**仅用于当前态/主按钮/焦点环/关键数；工作台卡片/资讯条/顶栏入口收口为实色面+发丝框（hover 聚光/active 压入/焦点描边）。`main.js` 于 `mobile-product.css` 后引入；SW `v110→v115` 清旧缓存。构建 `vite build dist2` 核验 `--dp-bg/--dp-line/--dp-accent: #4fc6b8` + SW v115 →替换 dist→`deploy_frontend.py`；**生产核验 PASS**（服务器 sw.js `xuanhuang-v115`、desktop token 进 `index-6eD2JQii.css`、home=200），桌面 Playwright 1440×900 深/日双主题截图验收卡片/表格收口生效、`#app.is-mobile` 门控零回归。仅前端，见 WORK_LOG 09-18、CHANGELOG v2.30.0。

- **2026-09-18**：**审 Codex 新手机 UI 去坑 + 产品化收敛（SW `xuanhuang-v110`，踩坑速查新记）**——Codex 配三方中转慢，改交付一版「产品化收敛层」`mobile-product.css`（把各模块统一成可靠数据工具、去模块渐变/玻璃碎片、注释自述须为唯一最终覆盖层）；接棒**完整核对**时发现它的前提没被满足、与旧 `mobile-module-color.css`「灵动·独立渐变」层**同载冲突**：后者选择器 `#app.is-mobile .fin-page .fin-kpi-val`(1,3,0) 精度高于前者 `#app.is-mobile .fin-kpi-val`(1,2,0)，即使收敛层后载也被渐变压住 → 记账页**零值仍被渐变/语义色渲染**（本月支出 ¥0 青绿渐变、流水笔数 0 带色）。**修 3 处硬坑 + 1 缓坑**：① `countdown.js` BASE 被误改 `/api/countdowns`→`/countdowns`（git 历史明示「路由加 /api 前缀」，dev/prod 均无 rewrite）改回，否则倒计时全端 404；② `MobileWorkbenchHome` 声明 `count` 未在 `onMounted` 赋值→「流水笔数」恒「—」，补 `d.month_count`；③ **删 `mobile-module-color.css`**（main.js 引用+文件本体，并清 `mobile-native-glass.css` 失效注释）→ `mobile-product.css` 成唯一收敛层、根治零值异色；④ SW 版本号+静态缓存同一版本不清旧缓存（PWA/APK 会拿旧 bundle）→ SW `v109→v110`。**核心理念沉淀**：两套同目标 CSS「收敛层」叠承载必被高特异性旧层反压——**要收敛就删掉旧层，让收敛层真正唯一**，不要指望后加载能赢异性选择器；另有「UI 重设计若顺手动到 API 前缀会静默打断全端请求」「新组件新增 KPI 字段别忘在数据回填处接线」两类回归点。构建 `npx vite build --outDir dist2` 核验与生产浏览器取证（记账三零值 `#f0f5f8` 中性色、流水笔数「—」→「0」、桌面零回归）均 PASS，见 WORK_LOG 09-18。
...
| 32 | **验收子页误用错误路由路径**：浏览器访问 `/tools` 而非真实 `/tool`，`vue-router` 无匹配回退到 `/workbench`，拿到的是工作台而非工具页，据此判样式「未生效」形成**假阴性** | 只凭一次"元素 not found"误报 P3 工具页隐藏未生效 | 验收前先核对 `router/index.js` 的真实路径（工具页是 `/tool` 单数）；校验隐藏用 `getComputedStyle(el).display`，元素不存在要区分「路由错了」还是「真没渲染」；受限浏览器截图 API 不可用时就以 computed display/offsetParent 为准，不需强求截图 |
| 33 | **CSS 规则校验误命中模板字符串**（见 #30 同类）：主 `index-*.js` 是入口 bundle，按路由拆分的 `.vue` page chunk 才含各页模板（本次 `.mu-admin-tools`/`.tl-admin-tools` 只在 `MusicView-*.js`/`ToolView-*.js`） | 只在 `index-*.js` 里 grep 包裹类报 MISS，误以为改动没进 build | 校验 Vue 模板产物要**扫 `assets/*.js` 全量**（或用 `rg "类名" dist2/assets -l`），CSS 规则则直接 grep 产物 `.css`；二者通道不同 |
| 30 | **前端「bundle 含标记」校验误判**：用 `rg -l "st-card-h" 产物/*.js` 查「CSS 是否部署成功」时，命中的是 **Vue 模板里的 class 字面量字符串**（`class="st-card-h"` 等），而非真的代表那条 CSS 规则进了产物 | 误以为去古风样式已上线，实测 DOM 却找不到对应隐藏 | 校验 CSS 规则必须**直接 grep 产物 `.css` 里的精确选择器**（含 `#app.is-mobile …` 前缀、`display:none`），模板字符串不能作证；PowerShell 用 `$text.Contains(...)`/`Select-String` 时注意 `-SimpleMatch` 与正则转义冲突 |
| 31 | **SW 版本未升导致旧缓存壳滞留**：改动仅 CSS/JS、`sw.js` 字节不变时浏览器不会重新安装 SW，沿用旧缓存命名空间命中旧 `index.html` → 用户看旧构建 | 部署后部分用户仍见旧界面，与服务器文件不符 | **每次前端部署都升 `public/sw.js` 的 `const VERSION = 'xuanhuang-vNN'`**（install/activate 清旧缓存存储桶）；本次特地从 v105→**v106** 补救后再重部署；部署后 `curl https://站点/sw.js` 核对版本号 |
