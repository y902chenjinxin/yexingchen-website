...
| 2026-09-06 | 待提交 | **全站「向晚·雨青」苹果毛玻璃质感升级（见方案 `GLASSMORPH_UPGRADE_20260906.md`，User「样式先优化，做成苹果毛玻璃那种质感」）**：以登录页玻璃为基准，全站统一夜色玻璃工艺——① `variables.css` 追加「向晚·雨青」配色（墨青夜色 `#0b0f14/#10161d/#161e26` 为骨、雨青 `#7fa8a3/#a8d3ce` 为光、收敛鎏金 `#c7a96b` 点睛），定义苹果级玻璃系统（`--glass-blur: blur(24px) saturate(185%)`、`--glass-border`、内高光 `--glass-highlight`、柔和多层 `--glass-shadow`），圆角放大（sm8/base14/lg20/xl26）；工作台原浅色 token（`--lj-*`）随动转深色玻璃，岛屿 `--ls-*` 对齐雨青；② `main.css` `.ls-card` 升级为苹果玻璃 + 新增通用 `.glass-card`，加 `prefers-reduced-motion` 克制动效；③ `xiuxian-theme.css` 覆盖 Element Plus 主色 `#7fa8a3`、夜色背景、大圆角、柔和多层阴影；④ 应用面：`WorkbenchView`（夜色光斑底+快捷动作玻璃化+深色页脚）、`GlobalTopBar`（导航/搜索/图标按钮夜色玻璃）、`LoginView`（基准玻璃参数深化）、`JadeCarousel` 分页指示器、`NoteSelect` 下拉夜色化、`App.vue`/`WorkbenchView` 页脚切 `site-footer variant="dark"`。SW 升 `xuanhuang-v34`。上线取证补 1 项：登录输入框原硬编码白色实底 `rgba(255,255,255,.95) !important`，与深色玻璃卡割裂，已改为夜色玻璃输入框（computed `rgba(24,32,41,.7)` + `blur(10px)` + 雨青描边、浅色文字/占位），现网复验无白色实底残留。
| 2026-09-06 | 待提交 | **登录页改版：去门 + 雨青粒子网络（User「喜欢博客那种鼠标移过就有棍状线条聚拢」「直接打开不要卡顿」）**：① **移除全部背景简单特效**——冰晶琉璃双开门、星空、云雾、门缝寒气、光环涟漪、粒子风、画面震动、空间扭曲波纹等 DOM 与对应动画脚本时序/辅助函数（`getStarStyle/getMistParticleStyle/getWindParticleStyle/getWaveRingStyle` 等）整体删除，`LoginView` 由 ~1200 行精简至 ~700 行；② **新增 `.particle-bg` canvas 经典粒子网+鼠标聚拢**——雨青粒子漂移（克制 0.32 速）、粒子间距离 130px 内按 `(1-d/L)×0.15` 连极细雨青线、鼠标 190px 影响半径内吸引粒子 + 加亮连线向光标聚拢（交互后 24.7% 像素变化），粒子带径向微光晕、核为冷白青，`prefers-reduced-motion` 时退化为静态帧，`onBeforeUnmount` 取消 rAF/解绑监听；③ **卡片直接淡现去卡顿**——登录玻璃卡 `card-in` 0.5s 纯 `opacity` 动画（0.15s 延迟、去 translateY、无 blur 动画，避免逐帧重绘卡顿），`transform: none`；登录「仙气飘飘」加载特效收敛为中央雨青呼吸光 + 旋转细环；④ **遗留清理**——删除空临时文件 `backend/_t.txt` 与 `backend/_test_output.txt`；`test_auth_service/music_stream/security` 三个回归测试**保留**（关联遗留 P1 002.1）。生产浏览器清 SW 取证 **6 项全 PASS**：11 个旧特效选择器 0 残留、canvas 中心 49% 非透明雨青像素、鼠标移动前后 24.7% 像素变化（聚拢生效）、`card-in` 仅 opacity 0→1 无位移、登录回归进 `/workbench`。
| 2026-09-06 | 待提交 | **移除「洞天将开」开场动画（User「洞天将开的动画也干掉」）**：删除 App 启动前的 `LoadingView` 全屏 canvas 蛇形开场（灵蛇入境/凝聚灵气/洞天将开三段动画）——`App.vue` 移除 `LoadingView` 组件/import/`onInitialLoadingComplete`，鉴权就绪后即 `showInitialLoading=false` 直接进入应用（保留净画布 gate 防内容闪现，登录态无闪烁）；`LoadingView.vue` 已删除。生产浏览器清 SW 取证 **PASS**：首屏 canvas 仅 1（粒子网）、无全屏动画层、无「洞天将开」文字，登录回归进 `/workbench`。
| 2026-09-06 | 本轮 | **全站搜索 + 独立原生 AI 助手（见方案 `SEARCH_GLOBAL_AND_NATIVE_AI_20260906.md`，SW `xuanhuang-v36→v38`）**：① **全站搜索统一**——后端 `/api/search` 追加笔记（标题+正文、owner 限定、排除软删），整合音乐/小说/视频/工具多模块；顶栏 `GlobalTopBar` 搜索提示语改「搜索全站：笔记 · 音乐 · 小说 · 视频 · 工具」，下拉划分为模块分组结果 + 底部「快速前往」常驻快捷入口（AI助手/音乐/小说/视频/工具），移除原「最近」/「标签」分组以空出展示区。② **独立原生 AI 助手**——后端新增直通流式端点 `POST /api/workbench/ai/chat/stream`（SSE，`HttpProvider.stream_chat` 原生多轮对话，用户/助手消息自动落库、`AiConversation` 历史可回溯，未配 Provider 回落 `FakeProvider` 离线演示），解除与笔记/能力清单的捆绑；前端 `AssistantView` 重写为原生对话（流式打字机逐字输出、左栏历史对话/新对话、Provider 下拉切换、气泡内「记入笔记」可折叠操作按需生成笔记，全程不弹窗）。③ **搜索下拉渲染缺陷修复**（生产浏览器四轮取证）——`el-input` 的 `@focus` 在真实点击/清缓存场景下未可靠同步 `searchFocus`，面板条件 `v-if="searchFocus && searchWord.trim()"` 恒不成立 ⇒ `.tb-suggest` 不挂载（API 却 200、值已录入）；修复：输入框补 `@mousedown="searchFocus=true"` 双触发，`onDocDown` 加固为「点击 `.tb-search`/`.tb-suggest` 内不收起」。**部署踩坑**：`npm run build` 对 `frontend/dist` 存在陈旧缓存（产出旧 bundle 缺新组件字符串，仅 `--outDir dist2` 全新构建才是正确产物）——本轮以「构建到全新 outDir→核验标记→整体替换 dist→再部署」规避，`/health` 200、生产 index bundle 核验含新标记。生产浏览器清 SW 取证：搜索面板弹出并含分组结果 + 快速前往、可进入 `/music` 页面、AI 流式打字机输出 + 记入笔记可用，全 PASS。 |
| 2026-09-06 | 待提交 | **顶栏遮挡页头终极修复：非 Island 独立页补 84px 顶距（SW `xuanhuang-v40`，User「新开无痕仍没有 / 笔记仍无返回」）**：浏览器复验发现上一条 v39 归因「陈旧 bundle」不完整——**固定顶栏（`position:fixed;height:60px`）覆盖了 `padding-top:24px` 的独立页页头**，入口/返回钮其实在 bundle 里，只是被顶栏盖住（无痕/无缓存同样被盖，故「重登仍没有」）。全局 `<router-view>` 无顶距偏移，非 `IslandInnerBase` 包裹的独立页必须自带 ≥60px 顶距。修复：7 个独立页根容器顶距 24px→**84px**——`NotesView`、`NoteEditorView`、`AssistantView`（页头含「选择 Provider + 配置 AI」）、`TrashView`、`AdminView`（`.admin-page` 补 `padding:84px 0 40px`，含「← 返回工作台」）、`AssetsView`/`TasksView`（路由已移除的遗留页，同步无害）。生产浏览器逐页截图取证 **6 项全 PASS**：工作台 AI 卡、`/assistant` Provider+配置 AI、`/notes`/`/notes/:id` 返回钮、`/admin` 返回工作台，均可见不被顶栏遮挡。 |
| 2026-09-06 | 待提交 | **四入口收敛 + 顶栏遮挡修复（SW `xuanhuang-v39`，User 五项清单）**：此前 v39 先做入口还原，因仅调整 bundle 未解决顶栏遮挡，后续 v40 补 84px 顶距才彻底修复（详见上一条）。① **AI Provider 配置/切换入口**——`AssistantView` 源码本就含页头「选择 Provider」下拉 +「配置 AI」按钮（Provider 列表 CRUD：添加/编辑/删除/设为默认/测试），生产此前为**陈旧 bundle** 导致该入口不渲染；本轮全新构建 re-deploy 后入口恢复，浏览器实测点「配置 AI」弹出「配置 AI Provider」面板、含既有 minimax provider、可增删改。② **管理页返回按钮**——五模块页（music/novel/video/tool/notes）页头均有「← 返回工作台」返回钮（模块页由 `IslandInnerBase` 内建 back-btn；notes 用 `BackButton`），生产浏览器逐页取证均有返回钮；此前"看不到"同为旧包所致。③ **顶栏去「笔记」导航**——`GlobalTopBar.navItems` 清空、`<nav>` 加 `v-if="navItems.length"` 收敛空导航，顶栏仅剩 品牌+搜索+用户区，浏览器实测无独立「笔记」菜单项（搜索框 placeholder 含"笔记"字样属占位提示，非导航）。④ **工作台去「快速记录」**——`WorkbenchView` 删「快速记录」快捷卡（连带删未用 `Edit` 图标 import），快捷动作仅剩「AI 助手」，浏览器实测正文「快速记录」0 命中。部署沿用「全新 outDir 构建→核验标记(含配置 AI/选择 Provider/无快速记录)→替换 dist→部署」，生产**清 SW 浏览器取证 4 项全 PASS**；浏览器验收用临时 super_admin 账号（`trae-browser-test@example.com`）插入生产 `yexingchen.db` 登录取证，验证毕即删除清理。 |

| 2026-09-06 | 本轮 | **PC 功能扩展 · 首页地基 + 语音基础设施（方案 `PC_EXPAND_PLAN_20260906.md` v1.1 定稿 D6，SW `xuanhuang-v42`，User「继续往下做」）**：多轮追问拍板 5 大功能一起做、首页骨架定案——① **工作台首页地基**：保持玉简轮播（8 张分类导航不动）+ 新增「常用工具区」（本轮固定 6：4 内置工具 + AI 对话/记一笔，去掉无功能「添加/管理」占位 + 移除失效 tone 样式）+ 新增「数据一览」大看板（账本/资讯/行情三卡并排，无数据时空态占位「还没有流水/暂无订阅源/还没有自选股」，点查看全部进施工页）+ 夜色页脚；② **顶栏收敛**：`GlobalTopBar.navItems` 仅留「AI 对话」（原笔记位），其余导航清空；③ **全局语音输入基础设施（Web Speech API）**：新建 `composables/useVoiceInput.ts` + `components/VoiceInputButton.vue`（🎤 按钮、录音中波浪动画+「正在聆听」提示、点击即停、不支持自动隐藏 feature detection），接入顶栏搜索（语音并入搜索词即触发联想/跳转）、AI 对话主输入框（并入草稿即发送）、笔记标题（语音即存）；④ 新路由 `{/finance,/feeds,/stocks,/stocks/:code,/tool/idphoto}` 指向通用 `BuildingView` 施工页。部署沿用「全新 outDir 构建→核验标记→替换 dist→部署」；**生产桌面宽浏览器取证**（Playwright 1440px 系统 Edge）：顶栏「AI 对话」可见（`.tb-nav` flex）、搜索框 🎤、常用工具 5 卡（缺证件照——内置工具尚未注册，注册后即 6）、数据看板三卡空态全 PASS；临时 `trae-browser-test` 账号密码已重置用于取证。 |

---

## 一、交付时间线二、关键交付明细（结论 + 验收）
