...
User：「我在下面切歌，上面高亮不切换，播放框中歌曲不跟着切换」「播放栏右边短了，没看进度条都出去了嘛」。

**问题 1（高亮不切换）根因**：
之前表格行只有"播放按钮变 ❚❚"区分当前曲目，但 Element Plus 的 stripe 隔行底色干扰视觉，用户感知不到当前在听哪首。

**修复**：
- `MusicView.vue` 加 `row-class-name`：当前点播行 `row-now-playing`、当前 BGM 行 `row-bgm-active`
- 行底色淡蓝、左 3px 强色条（点播蓝 / BGM 橙），hover 时颜色加深
- 视觉上无论隔行条纹都能一眼看出

**问题 2（播放框右边溢出）根因**：
之前 `width: calc(100% - 320px)` 假设有 320px 侧栏，但 MusicView 这类 island 全屏页面没有侧栏，导致宽度算大、超出视口右边界，进度条/音量/关闭按钮跑到表格下面被"设为默认"列遮住。

**修复**：
- `NowPlayingBar.vue` 改为 `left: 50%; transform: translateX(-50%)`，`width: min(calc(100% - 32px), 1200px)`
- 不再依赖任何侧栏宽度，居中铺开；MusicView 这类无侧栏页面也不会溢出
- 移动端样式同步居中
...
User：「已经在播放虫儿飞了，上面和下面都没切换名称，还是声声慢，需要优化」「这里进度条还是挤在一块了，并且歌名和歌手都没了，你咋做的优化，我要求的横向拉长，不是升高，给我换下」。

**问题 1（听觉视觉不一致）根因**：
之前 `NowPlayingBar` 显示条件只看 `player.shows`（= mode==='playlist' && curItem）；在 BGM 模式下点击"设为默认"切到新曲后，`audio.src` 已切但 store 的 `curItem` 不变（设计如此），导致播放栏继续显示旧 curItem → 视觉与听觉错位。

**修复**：
- 新增 `displayItem` computed：playlist 模式用 curItem；BGM 模式用 `bgmLibrary.musicLibrary[bgmChoiceId]`
- 显示条件改为 `player.shows || bgmItem`，BGM 模式下也持续显示
- 关闭按钮仅在 playlist 模式显示（BGM 模式下"关闭点播"语义不成立）
- 上下首/模式按钮在 BGM 模式下禁用

**问题 2（横向拉长 vs 升高）**：
之前误把"拉长"做成了"升高"（76px 高），导致按钮偏大、进度条仍挤。

**修正**：
- 容器高度 76 → **60px**（纵向压低）
- 标题/作者区宽度 240 → **280px**（歌名/歌手不再被裁掉）
- 主播按钮 48 → 40px，上下首/模式 38 → 32px（小一档让横向更舒展）
- 进度条 `flex: 1` 占满剩余宽度，是这次拉长的主力
- 容器最大宽度 1080 → 1200px
...
### 播放模式：点击展开 4 项列表直接选 + 播放框拉长

User：「播放模式在点击时加个列表，我直接点击选择」「这里播放框有些遮挡了，拉长一点吧」。

**改动**：
- `NowPlayingBar.vue`：模式按钮从「点击循环切换」改为「点击展开 4 项下拉菜单直接选」，含图标 + 模式名 + 当前项 ✓ 标识；从按钮上方弹出避免遮挡右侧
- `MobileFullPlayer.vue`：模式按钮同步改为下拉菜单（一致体验）
- 容器高度 62 → 76px；按钮 36 → 40px；主播放按钮 48px 带渐变阴影；上下首按钮 38px；模式按钮 38px 圆角；间距 14 → 18px；元数据 max-width 200 → 240px；时间 38 → 44px（避免贴在一起）
- 移动端布局同步调高（68px），按钮自适应缩小

**指令补全**：`main.js` 新增全局 `v-click-outside` 指令（之前引用但未注册，TravelCityPicker 实际未生效）
...
User：「这样一看工作台现有的布局就不好看了，有些信息在下面，有些信息被隐藏，能不能整合到一个一眼就能看全部的页面上来」。

之前工作台是**多段拼接**——Hero → KPI → 趋势 → AI 简报 → 习惯打卡 → 笔记/任务/标签 → 资讯流，下半页信息藏在滚动下方。整合成 **CSS Grid 12 列驱动舱布局**：

**首屏上半（S1，单屏可视）**
```
┌─────────────────────────────────────────────────────────────────────────┐
│  [KPI 4卡 2×2]    │     记账·最近30天趋势图       │   即将到来           │
│                   │                                │   ─────────         │
│                   │                                │   自选股            │
└─────────────────────────────────────────────────────────────────────────┘
```
...
**关键设计**：
- **0 隐藏**——首屏（≥1024px）能看到：4 个 KPI + 记账趋势 + 倒计时 + 自选股 + AI 简报 + 习惯打卡 + 笔记 5 条 + 待办 5 条 + 标签云
- **响应式退路**（1024px → KPI/趋势/侧栏 2 行；768px → 全单列）
- 标题字号统一 13px、行间距收紧（5~6px），信息密度提升但仍可读
- 自选股 + 倒计时合并右栏，省去原来两个独立 trend-card 的占位

## [v2.39.6] - 2026-09-21

### 工作台：天气并入驾驶舱中列 + 移除习惯打卡

User：「天气呢，另外不需要习惯打卡」。

v2.39.5 整合驾驶舱时把天气漏了（之前单独 section），也保留了用户不想要的习惯打卡模块。本次调整：

- **天气并入中列**：将 `<WeatherCard>` 从 hero 下方独立段移到「记账 30 天」下方堆叠，组成中列双段；缩小天气卡内部 padding/字体，避免与记账图视觉权重冲突
- **移除习惯打卡**：删 `<HabitsCard>`、删 import、删 CSS；个人中心开关仍保留（`prefs.moduleVisible.habits`），用户后续若想用可自行启用
- 响应式 grid-area 同步改为 `mid`

**结构（v2.39.6）**
```
S1 ┌─4 KPI 2×2─┐ ┌─记账 30 天──┐ ┌─即将到来─┐
   │           │ │ 天气（新增）│ │ 自选股   │
   └────────────┘ └─────────────┘ └───────────┘
S2 ┌─AI 简报─────────────────────────────┐
   ├─笔记─┬─待办─┬─标签─┐
...
### 工作台 Bento Grid 2.0 布局（GitHub 调研：Dashy/Bento/Bentofolio）

User：「去 github 上找找看，有没有好看的布局」「A」。

参考 GitHub 三大开源个人工作台范式（[Dashy 37k star](https://github.com/lissy93/dashy) / [Bento startpage](https://github.com/migueravila/Bento) / Bentofolio 2026 bento grid 趋势），选定 **方案 A：Bento Grid 2.0 不对称大块**——Apple/iOS 风格 + Quiet Luxury 内边距 + 圆角 20px，与现有 OLED 真黑 + 线性风完美融合。

**结构（v2.39.7）**
```
S1 首屏核心数字区（2 列 Bento + 不对称大块）
┌───────────────────────┬───────────────────────┐
│ ╔═══════════════════╗ │   记账·30 天趋势       │
│ ║  本月净流入        ║ │   （大块 2 行高）       │
│ ║  ¥+4,283          ║ ├───────────────────────┤
...
S2 等宽四卡：自选股 / 最近笔记 / 今日待办 / 标签
S3 AI 简报（独占大块）
S4 资讯流
```

**关键设计**：
- **不对称**：「净流入」横跨 2 列 + 占 2 行高 → 大字号 48px 数字，**视觉焦点**；旁边的「记账趋势」同样占 2 行高
- **小 KPI 紧凑**：足迹 / 笔记是小方块（28px 数字），与 Hero 大块对比
- **倒计时嵌左下**：横跨 2 列横排列表，不再独立 card
- **天气嵌右下**：和记账趋势一起堆叠右列
- **圆角统一 20px / 16px**（大块 20、小块 16）；卡片间距 16px；hover 紫罗兰描边 + translateY(-1px)
- **响应式**：1280px 起 KPI 列变窄 + S2 4→2 列；1024px 起 S1 单列堆叠；768px 起 Hero 数字 48→36px

## [v2.39.8] - 2026-09-21

### 工作台精简 + 修自选股不显示 + AI 简报刷新优化

User：「1. 笔记入口太多了，右下角两个点击挑战笔记的不要；2. 我有自选股，但是首页没展示；3. AI 简报刷新没有效果，需要优化下」「这两个（最近笔记/标签·主题）也去掉，重复了」。

**改动**：
- **移除右下角 AI FAB**（跳 `/assistant`）：改由顶栏 ✺ 按钮 + Cmd/Ctrl+Shift+A 全局快捷键直达（保留桌宠 WhaleCompanion 作为装饰）
- **移除 S2「最近笔记」+「标签·主题」两个 card**：已有侧栏导航 / ⌘K 命令面板 / 顶栏 ✺ 三条进入笔记的入口，工作台再放两个仅展示空态（数据为 0）的卡是冗余
- **修自选股不显示 bug**：`stores/workbench.js` 的 `loadSummary()` 没 `return res.data`，导致 `WorkbenchView` 里 `await wbStore.loadSummary()` 拿到 `undefined`，`s.stocks` 取值失败 → 自选股永远显示「尚未添加」。修：增加 `return res.data`
- **修 AI 简报刷新无效果**：
  - 强制最少显示 600ms loading（避免 fake 模式快响应"看起来没反应"）
  - 加 `ElMessage.success('已刷新')` 反馈；失败显示具体原因（未配置 Provider 等）
- **S2 改 2 列**（自选股 1fr : 待办 1.4fr），自选股更大比例
